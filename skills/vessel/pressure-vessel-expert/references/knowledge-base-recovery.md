...
## 8. 取总页数的 OOM bug（2026-06-30 修复）

**症状**：脚本在调用 `extract_with_ocr` 时，**第一行取总页数**就 OOM。哪怕 PDF 不到 100MB 也可能触发。

**根因**：`process_pdf_batch.py` 第 73 行（修复前）用无页数限制的 `convert_from_path` 取总页数：
```python
# ❌ 修复前: 一行就把全本 PDF 加载进内存
total = len(convert_from_path(str(pdf_path), dpi=dpi))  # 先拿总页数
```
**461 页 × 200 DPI × A4** ≈ 5GB+ 内存，触发 OOM Killer。

**修复后**（已写入 `process_pdf_batch.py`）：用 `pdfinfo` 取总页数，零内存开销：
```python
# ✅ 修复后: pdfinfo 是文本流解析, 内存开销可忽略
...
**实测**：GB150 第五版 80MB / 461 页修复后能正常启动 OCR 流程, 内存峰值稳定。
...
**史鉴** (2026-06-30): Sir 上传新 GB150 扫描版, 旧脚本第 73 行直接 OOM, 修复后启动后台 OCR 任务 (PID 1288406, 预计 30-60 分钟)。
